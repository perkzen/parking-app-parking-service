...
public class ParkingSpotRepository implements ReactivePanacheMongoRepository<ParkingSpot> {
    ...
    public Uni<ParkingSpot> getRandomParkingSpot() {
        var parkingSpot = listAllParkingSpots();
        return parkingSpot.map(parkingSpots -> parkingSpots.get((int) (Math.random() * parkingSpots.size())));
    }
    ...
    public Uni<Void> freeOrOccupy(String name, String car) {
        return getParkingSpot(name)
                .onItem().transformToUni(parkingSpot -> {
                    if (parkingSpot.occupied) {
                        return free(name);
                    } else {
                        return occupy(name, car);
                    }
                })
                .replaceWithVoid();
    }
